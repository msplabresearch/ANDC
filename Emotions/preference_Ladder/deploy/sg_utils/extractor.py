...
class HLDextractor:
    def __init__(self, *args, **kwargs):
        ...
        self.feat_type = kwargs.get("feat_type", "IS13")
        self.opensmile_path = kwargs.get("opensmile_path", "/media/kyunster/hdd/Project/opensmile")
        if self.feat_type == "IS13":
            self.config_path = os.path.join(self.opensmile_path,"config", "IS13_ComParE.conf")

    def extract_all(self, raw_wav_list):
        feat_list = []
        for raw_wav_path in tqdm(raw_wav_list):
            utt_id = raw_wav_path.split("/")[-1].replace(".wav",".txt")
            out_path = self.temp_path+"/"+utt_id
            cmd = "SMILExtract -C "+self.config_path+" -I "+raw_wav_path+" -O "+out_path+" -l 0 || exit 0;"
            os.system(cmd)
            ...
            with open(out_path, 'r') as f:
                ...
                for line in f:
                    continue
            hld = [np.round(float(val), 4) for val in line.split(",")[1:-1]]
            assert len(hld) == 6373
            feat_list.append(hld)
