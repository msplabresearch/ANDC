...
def MTL_loss(y_pred, y_true, abs_coef=0.25, rank_coef=0.5):
    print("Y_pred: ", y_pred)
    abs_pred1 = float(y_pred[0])
    abs_pred2 = float(y_pred[1])
    rank_pred = float(y_pred[2])
    
    abs_true1 = float(y_true[0])
    abs_true2 = float(y_true[1])
    rank_true = float(y_true[2])
	
    ccc1 = cc_coef(abs_pred1, abs_true1)
    ccc2 = cc_coef(abs_pred2, abs_true2)
    rank = K.binary_crossentropy(rank_pred, rank_true)
    loss = abs_coef * ccc1 + abs_coef * ccc2 + rank_coef * rank
    return loss
def cc_rank_loss(y_true, y_pred):
