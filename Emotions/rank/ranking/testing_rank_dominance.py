...
model_path = os.path.join(file_dir, 'Models','LSTM_model[epoch15-batch128-lr5e-05]_RnnAttenVec_Dom_weights.h5')
...
#         with open(root_dir + test_file_path[i].replace('.wav', '.pk'), 'rb') as f1:
#             data = pk.load(f1)
...
#         print(pred[0])
#         f.write(test_file_path[i] + ' ' + str(pred[0])+'\n')

# Test_pred_Val = []
# File_Name = []


print("RankNet Dominance inference: ")
for filename, f_info in tqdm(audio_files.items()):
    ...
    chunk_data = DynamicChunkSplitTestingData(data1, m=50, C=11, n=1)
    pred = intermediate_layer_model.predict([chunk_data, np.zeros_like(chunk_data)])
    f_info['ranknet_dominance_pred'] = float(pred[0][0])
...
print("Finished RankNet Dominance")