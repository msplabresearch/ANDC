...
class WavSet(torch_utils.data.Dataset):
    def __init__(self, *args, **kwargs):
        ...
        self.utt_list = kwargs.get("utt_list", args[1])
    ...
    def __getitem__(self, idx):
        ...
        if self.print_dur:
            result = result + (cur_dur,)
        if self.print_utt:
            result = result + (self.utt_list[idx],)
        return result
...
def collate_fn_padd(batch):
    ...
    for cur_batch in batch:
        ...
        total_lab.append(cur_batch[1])
        total_dur.append(cur_batch[2])
        if enable_utt:
            total_utt.append(cur_batch[3])
