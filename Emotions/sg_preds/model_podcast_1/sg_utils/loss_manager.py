...
class LogManager:
    ...
    def add_torch_stat(self, stat_type, stat):
        assert stat_type in self.log_book, "Wrong stat type"
        if stat_type in ['train_acc','test_acc', 'dev_acc']:
            self.log_book[stat_type].append(float(stat))
        else:
...
def scores(results, truths):
    ...
    predictions = (test_preds>threshold).float()*1

    # print(predictions)
    
    test_preds_i = np.array(predictions)
